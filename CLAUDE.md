...
- To test an upgrade from a released version, build its tag (`git switch --detach <tag>`, `./gradlew :mobile:assembleDebug`, copy the APK out before switching back and rebuilding, as both write `mobile-debug.apk`), install it over the current one with `adb install -r -d` (downgrade, keeps data), use it, then `adb install -r` the new build.
- Turn off a single notification channel with `adb shell am start -a android.settings.CHANNEL_NOTIFICATION_SETTINGS --es android.provider.extra.APP_PACKAGE com.everlog.dev --es android.provider.extra.CHANNEL_ID <id>` and tap its toggle (`uiautomator dump` for the bounds). Check it with `dumpsys notification` (`mImportance=0`).
